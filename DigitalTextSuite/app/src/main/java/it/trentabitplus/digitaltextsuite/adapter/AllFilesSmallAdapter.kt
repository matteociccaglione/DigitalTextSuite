...
class AllFilesSmallAdapter(val listNotes: List<Note>, val context: Context,val handler: SelectedHandler): RecyclerView.Adapter<SmallFileViewHolder>() {
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): SmallFileViewHolder {
        ...
        return SmallFileViewHolder(binding,context)
    }
...
class SmallFileViewHolder(val binding: ItemFileSmallBinding,val context: Context): RecyclerView.ViewHolder(binding.root){
    var note: Note = Note()
    set(value){
        field = value
